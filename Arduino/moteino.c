...
void loop()
{
	if (radio.receiveDone()) // checks if packet was received and/or puts transceiver in receive (ie RX or listen mode)
	{
...
void handleProgram()
{
	if (programLength > 0)
	{
		...
		{
			programZoneStart = millis(); //mark start time of a zone, millis is an Arduino function
